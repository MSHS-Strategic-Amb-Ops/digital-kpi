...
appt_online_physician_summary <- appt_online_physician %>%
  ...
  pivot_wider(names_from = c("online_exclude_criteria", "avail_online", "sched_online"),
              ...
              values_fill = 0) %>%
  mutate(perc_avail_online_all = (EXC_avail_Y_sched_N + EXC_avail_Y_sched_Y + INC_avail_Y_sched_N + INC_avail_Y_sched_Y) / 
...
reactable(
  ...
  columnGroups = list(
    ...
    colGroup(name = paste0("All Visits"),
             columns = c("perc_avail_online_all", "perc_sched_online_all"),
             headerStyle = list(background = "#212070", color = "white", fontSize = "15px")),
    colGroup(name = paste0("Visits in Scope"),
             columns = c("perc_avail_online_inScope", "perc_sched_online_inScope"),
