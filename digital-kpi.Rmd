...
options(dplyr.summarise.inform = FALSE, width = 120)
```
<style type="text/css">
div.main-container {
  max-width: 2000px;
...
```{r Graph asthetics, echo = FALSE, warning = FALSE, message = FALSE, include=FALSE}
...
```{r Global Functions, echo = FALSE, warning = FALSE, message = FALSE, include=FALSE}
...
```{r Data Connections, echo = FALSE, warning = FALSE, message = FALSE, include=FALSE}
...
conn5 <- dbConnect(odbc(),
                    ...
                    UID = Sys.getenv("DATAHUB_UID"),
                    PWD = Sys.getenv("DATAHUB_PWD"),
                    Port = 1433)
...
```{r Report Variables, echo = FALSE, warning = FALSE, message = FALSE, include=FALSE}
...
prior_month <- report_run_date - months(1)
prior_month_label <- format(report_run_date - months(1), "%Y-%m")
```


```{r Digital KPI In Scope Criteria, echo = FALSE, warning = FALSE, message = FALSE, include=FALSE}
...
#   filter(year(CONTACT_DATE) >= 2023) %>%
#   filter(year(CONTACT_DATE) <= 2025) %>%
#   filter(DERIVED_STATUS_DESC == "Arrived") %>%
#   group_by(year(CONTACT_DATE)) %>%
#   summarise(unique_pts = n_distinct(MRN),
#             unique_tele = n_distinct(PAT_ENC_CSN_ID[VISIT_METHOD != "IN PERSON"])) %>%
#   collect()
#   
...
```{r Digital Enablement Status, echo = FALSE, warning = FALSE, message = FALSE, include=FALSE}
...
digital_status <- access_raw_tbl %>%
  left_join(tbl(conn2, "CLARITY_SER_DEPT") %>%
              dplyr::select(PROV_ID, DEPARTMENT_ID, INACT_CAD_DEPT_YN)) %>%
  filter(is.na(INACT_CAD_DEPT_YN) | INACT_CAD_DEPT_YN == "N") %>%
...
```{r Online Appointments Physicians, echo = FALSE, warning = FALSE, message = FALSE, include=FALSE}
...
appt_online_physician <- access_raw_tbl %>%
  ...
  group_by(SITE, SPECIALTY_GRP, DEPT_SPECIALTY_NAME, DEPARTMENT_NAME, DEPARTMENT_ID, PROV_TYPE, PROV_NAME_WID, PROV_ID, ACTIVE_STATUS, appt_made_year, appt_made_month, 
           VISIT_PROV_STAFF_RESOURCE_C, VISIT_METHOD, visit_type, PRC_NAME, PRC_ID, FINCLASS,
...
```{r Online Appointments Physicians Summary, echo = FALSE, warning = FALSE, message = FALSE, include=FALSE}
...
appt_online_physician_summary_commercial <- appt_online_physician %>%
  filter(avail_online_exclude == "N") %>%
  filter(FINCLASS == "COMMERCIAL_MANAGED_CARE") %>%
  group_by(SITE, SPECIALTY_GRP, DEPT_SPECIALTY_NAME, DEPARTMENT_NAME, DEPARTMENT_ID, PROV_TYPE, PROV_NAME_WID, PROV_ID, appt_made_year, appt_made_month,
           avail_online, sched_online, online_exclude_criteria, FINCLASS) %>%
  summarise(appts_sched = sum(total, na.rm = T)) %>%
  pivot_wider(names_from = c("online_exclude_criteria", "avail_online", "sched_online","FINCLASS"),
              values_from = appts_sched,
              values_fill = 0) %>%
  rename("Year" = appt_made_year,
         "Month" = appt_made_month)
...
appt_online_physician_summary_merged <- left_join(
  appt_online_physician_summary,
  appt_online_physician_summary_commercial,
  by = c("SITE", "SPECIALTY_GRP", "DEPT_SPECIALTY_NAME", "DEPARTMENT_NAME",
         "DEPARTMENT_ID", "PROV_TYPE", "PROV_NAME_WID", "PROV_ID", "Year", "Month")
)

location <- tbl(conn1, "EPIC_DEPT_LOCATION_MAPPING") %>%
  dplyr::select(STANDARD_ADDRESS, DEPARTMENT_ID) %>%
  distinct() %>%
  collect()

appt_online_physician_summary_merged <- appt_online_physician_summary_merged %>%
  left_join(location, by = "DEPARTMENT_ID")
...
```{r Online Appointments Physicians Criteria Summary, echo = FALSE, warning = FALSE, message = FALSE, include=FALSE}
...
```{r Online Scheduling Audit Data Pull, echo = FALSE, warning = FALSE, message = FALSE, include=FALSE}
...
```{r Prepare Display Data and Close Connections, include=FALSE}
prior_year_num <- as.numeric(format(prior_month, "%Y"))
prior_month_num <- as.numeric(format(prior_month, "%m"))

all_sites_display <- appt_online_physician_summary_status %>%
  filter(Year == prior_year_num, Month == prior_month_num) %>%
  dplyr::select(-Year, -Month, -SPECIALTY_GRP) %>%
  mutate(Total = "Total") %>%
  arrange(SITE, DEPT_SPECIALTY_NAME, DEPARTMENT_NAME, PROV_NAME_WID)

provider_display <- appt_online_physician_summary_merged %>%
  filter(Year == prior_year_num, Month == prior_month_num) %>%
  arrange(SITE, DEPT_SPECIALTY_NAME, DEPARTMENT_NAME, PROV_NAME_WID)

criteria_display <- appt_online_criteria_summary %>%
  filter(Year == prior_year_num, Month == prior_month_num)

rm(appt_online_physician, location, digital_status)
invisible(gc())

for (p in c("conn1", "conn3", "conn4")) {
  if (exists(p, inherits = FALSE)) try(pool::poolClose(get(p)), silent = TRUE)
}
for (con in c("conn2", "conn5")) {
  if (exists(con, inherits = FALSE)) try(DBI::dbDisconnect(get(con)), silent = TRUE)
}
invisible(gc())
```
...
## All: `r prior_month_label` 
```{r Prior Month Metrics - All Sites, echo = FALSE, warning = FALSE, message = FALSE}
...
htmltools::browsable(
  tagList(
    tags$button(
      ...
      onclick = "Reactable.downloadDataCSV('digital-metrics-all-sites', 'Online Scheduling by Site')"
    ),
...
reactable(
  all_sites_display,
  elementId = "digital-metrics-all-sites",
  ...
  groupBy = c("Total", "SITE", "DEPT_SPECIALTY_NAME", "DEPARTMENT_NAME", "PROV_TYPE", "PROV_NAME_WID"),

  columnGroups = list(
    colGroup(name = "", columns = c("Total", "SITE", "DEPT_SPECIALTY_NAME", "DEPARTMENT_NAME", "PROV_TYPE", "PROV_NAME_WID"),
...
```
<br/>
<p style="font-size:10pt; font-style:italic">% Available Online: The percentage of appointments that were available for online scheduling, out of all scheduled appointments.<p/>
<p style="font-size:10pt; font-style:italic">% Scheduled Online: The percentage of appointments which were scheduled through an online application, out of all appointments that could have been scheduled via direct, open or ticket scheduling.<p/>
<p style="font-size:10pt; font-style:italic">% Captured Online: Calculates the percentage of online available appointments that were successfully scheduled through online means.<p/>
<br/>

## All: Metrics by Provider
```{r Metrics by Provider, echo = FALSE, warning = FALSE, message = FALSE}
...
reactable(
  ...
  columns = list(
    ...
    INC_avail_Y_sched_N= colDef(show =F),
    INC_avail_Y_sched_Y = colDef(show =F),
    EXC_avail_N_sched_N_COMMERCIAL_MANAGED_CARE = colDef(show =F),
    INC_avail_N_sched_N_COMMERCIAL_MANAGED_CARE = colDef(show =F),
    INC_avail_Y_sched_N_COMMERCIAL_MANAGED_CARE = colDef(show =F),
    INC_avail_Y_sched_Y_COMMERCIAL_MANAGED_CARE = colDef(show =F),
    EXC_avail_Y_sched_N_COMMERCIAL_MANAGED_CARE = colDef(show =F),
    EXC_avail_Y_sched_Y_COMMERCIAL_MANAGED_CARE = colDef(show =F),
    STANDARD_ADDRESS = colDef(show =F)
...
## All: Metrics by In Scope Criteria
<br/>
<br/>
<p style="font-size:14pt; font-weight:bold">Visits In Scope:<br/><p/>
...
<p/>
<br/>
```{r Metrics by In Scope Criteria, echo = FALSE, warning = FALSE, message = FALSE}
