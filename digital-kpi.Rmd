...
appt_online_physician_summary_merged <- left_join(appt_online_physician_summary, appt_online_physician_summary_commercial)
...
appt_online_physician_summary_merged <- appt_online_physician_summary_merged %>%
  left_join(location %>% dplyr::select(STANDARD_ADDRESS, DEPARTMENT_ID))
...
## MSH, MSW/M, Downtown: `r prior_month_label` 
```{r Prior Month Metrics - MSH, MSW/M, Downtown, echo = FALSE, warning = FALSE, message = FALSE}
...
#       onclick = "Reactable.downloadDataCSV('digital-metrics-site', 'Online Scheduling by Site')"
#     ),
...
reactable(
  appt_online_physician_summary_status %>%
    filter(SITE %!in% c("MSH- AMBULATORY CARE", "MSDMG", "NETWORK")) %>%
    filter(Year == format(prior_month, "%Y")) %>%
    filter(Month == as.numeric(format(prior_month, "%m"))) %>%
    dplyr::select(-c(Year, Month, SPECIALTY_GRP)) %>%
    arrange(DEPT_SPECIALTY_NAME, SITE, DEPARTMENT_NAME, PROV_NAME_WID),
  # elementId = "digital-metrics-site",
  ...
  groupBy = c("DEPT_SPECIALTY_NAME", "SITE", "DEPARTMENT_NAME", "PROV_TYPE", "PROV_NAME_WID"),

  columnGroups = list(
    colGroup(name = "", columns = c("SITE", "DEPT_SPECIALTY_NAME", "DEPARTMENT_NAME", "PROV_TYPE", "PROV_NAME_WID"),
...
## MSH-AMB CARE: `r prior_month_label`
```{r Prior Month Metrics - MSH-AMB CARE, echo = FALSE, warning = FALSE, message = FALSE}
...
reactable(
  appt_online_physician_summary_status %>%
    filter(SITE == "MSH- AMBULATORY CARE") %>%
    filter(Year == as.numeric(format(prior_month, "%Y"))) %>%
    filter(Month == as.numeric(format(prior_month, "%m"))) %>%
    dplyr::select(-c(Year, Month, SPECIALTY_GRP)) %>%
  ...
  groupBy = c("SITE", "DEPT_SPECIALTY_NAME", "DEPARTMENT_NAME", "PROV_TYPE", "PROV_NAME_WID"),
...
## MSDMG: `r prior_month_label`
```{r Prior Month Metrics - MSDMG, echo = FALSE, warning = FALSE, message = FALSE}
...
reactable(
  appt_online_physician_summary_status %>%
    ...
    filter(Year == as.numeric(format(prior_month, "%Y"))) %>%
    filter(Month == as.numeric(format(prior_month, "%m"))) %>%
    dplyr::select(-c(Year, Month, SPECIALTY_GRP, DEPT_SPECIALTY_NAME)) %>%
    arrange(SITE, DEPARTMENT_NAME, PROV_NAME_WID),
  # elementId = "digital-metrics-site",
  style = list(fontFamily = 'Calibri',
                 fontSize = '14px'),
    defaultColDef = colDef(align = "left",
  ...
  groupBy = c("SITE", "DEPARTMENT_NAME", "PROV_TYPE", "PROV_NAME_WID"),

  columnGroups = list(
    colGroup(name = "", columns = c("SITE", "DEPARTMENT_NAME", "PROV_TYPE", "PROV_NAME_WID"),
             headerStyle = list(color = "black", fontSize = "15px"),
             sticky = "left"),
    colGroup(name = paste0("Open & Direct Scheduling Status"),
             columns = c("PROV_COUNT", "BOTH_OPN_SCHED", "BOTH_DSCHED", "BOTH_OPN_SCHED_DSCHED", "PERC_BOTH_OPN_SCHED", "PERC_BOTH_DSCHED", "PERC_BOTH_OPN_SCHED_DSCHED"),
...
## Network: `r prior_month_label`
```{r Prior Month Metrics - Network, echo = FALSE, warning = FALSE, message = FALSE}
...
reactable(
  appt_online_physician_summary_merged, 
